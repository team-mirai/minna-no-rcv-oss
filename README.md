...
- 指定した場合、締切済みでもその時刻まで **結果ページとプレゼンモードは結果を出しません**
  （主催者も同じ画面になります）。待機画面では集計そのものを呼ばないので、描画しない
  だけで props に載る（＝ DevTools から読める）事故も起きません。
...
表示は主催者と参加者で食い違わないよう **日本時間で固定** しています。結果を見せてよいかの
